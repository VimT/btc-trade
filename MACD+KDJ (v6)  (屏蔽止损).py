start_time = "2017-07-14 11:00:00"#time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()+2*60*60))
end_time = "2017-09-14 20:00:00"#time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()+22*60*60))
...
def KDJ_cross(K,D,J):
    if K[-2] < D[-2] and K[-1] >= D[-1] and D[-1] < 60:
        return 0
...
def main():
    ...
    while True:
        ...
        for i in parts:
            #买入条件
            if i.buy_flag == False and can_buy == True and select == 0:
