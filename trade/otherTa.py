    ...
    def ATR(records, period=14):
        ...
        for i in range(0, len(records)):
            ...
            if i == 0:
                ...
                TR = max(records[i]['High'] - records[i]['Low'], abs(records[i]['High'] - records[i - 1]['Close']),
                         abs(records[i - 1]['Close'] - records[i]['Low']))
            m += TR
            if i < period:
                n = m / (i + 1)
            else:
    ...
    def CMF(records, periods=20):
        ...
        for i in range(0, len(records)):
            ...
            if records[i]['High'] != records[i]['Low']:
                d = (2 * records[i]['Close'] - records[i]['Low'] - records[i]['High']) / (
