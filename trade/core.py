...
class Trade:
    def __init__(self):
        ...
        self.client = Client(self._access_key, self._secret_key, False)
    ...
    def GetRecords(self, period):
        ...
        for i in response:
            t = i[0]
            time = datetime(int(t[:4]), int(t[4:6]), int(t[6:8]), int(t[8:10]), int(t[10:12]), int(t[12:14]), int(t[14:]))
            result.append(Record(int(time.timestamp() * 1000), i[1], i[2], i[3], i[4], i[5]))
        return numpy.ndarray(result)
